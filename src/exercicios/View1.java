...
public class View1 extends javax.swing.JFrame {
    ...
    private void btnSortearActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSortearActionPerformed
        sorteioText.delete(0, sorteioText.length());
        int i = 0;
    ...
    private void btnGravarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGravarActionPerformed
        try {
            arq = new FileWriter("d:\\sorteio-" + id + ".txt");
            gravarArq = new PrintWriter(arq);
            gravarArq.printf("Números sorteados: " + sorteioText + "\n\n");
            for (int i = 0; i < tbCadastro.getRowCount(); i++) {
                String dados = "Nome: " + String.valueOf(model.getValueAt(i, 0))
                        + "\nPalpite: " + String.valueOf(model.getValueAt(i, 1))
                        + "\nDiferença: " + String.valueOf(model.getValueAt(i, 2))
                        + "\nNúmero + Próximo: " + String.valueOf(model.getValueAt(i, 3))
                        + "\n\n";
                gravarArq.printf(dados);
            }
            
            File file = new File("d:\\sorteio-" + id + ".txt");
        ...
        try {
            int linha = tbCadastro.getSelectedRow();
            if (linha != -1) {
                arq = new FileWriter("d:\\palpite-" + (linha + 1) + ".txt");
                gravarArq = new PrintWriter(arq);
                gravarArq.printf("Números sorteados: " + sorteioText + "\n\n");
                String dados = "Nome: " + String.valueOf(model.getValueAt(linha, 0))
                        + "\nPalpite: " + String.valueOf(model.getValueAt(linha, 1))
                        + "\nDiferença: " + String.valueOf(model.getValueAt(linha, 2))
                        + "\nNúmero + Próximo: " + String.valueOf(model.getValueAt(linha, 3))
                        + "\n\n";
                gravarArq.printf(dados);
                arq.close();
            }
